...
class RunMigrationTests(unittest.TestCase):
    def test_v1_migrates_in_memory_to_historical_v3_without_mutating_source(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            source.pop("origin")
            source.pop("automation")
            source.pop("admission_sha256")
            original = copy.deepcopy(source)
            migrated = migrate_run_document(source)
        self.assertEqual(source, original)
        self.assertEqual(migrated.source_version, 1)
        self.assertEqual(migrated.target_version, 3)
        self.assertEqual(migrated.applied, (1, 2))
        ...
        self.assertEqual(migrated.document["origin"], {"kind": "manual", "trigger": "manual", "reason": None})
        self.assertIsNone(migrated.document["automation"])
        self.assertRegex(migrated.document["admission_sha256"], r"^[0-9a-f]{64}$")

    def test_future_and_invalid_versions_are_rejected(self):
        for value, code in ((4, "future_run_schema_version"), (True, "invalid_run_schema_version"), ("1", "invalid_run_schema_version")):
    ...
    def test_store_read_does_not_rewrite_but_later_mutation_persists_v3(self):
        with tempfile.TemporaryDirectory() as temporary:
            store = BuildStore(Path(temporary) / "builds")
            ...
            self.assertEqual(persisted["origin"]["kind"], "manual")

    def test_schema_downgrade_cannot_erase_automation_provenance(self):
        with tempfile.TemporaryDirectory() as temporary:
            store = BuildStore(Path(temporary) / "builds")
            run = store.create(recipe(), run_id="downgraded")
            run["schema_version"] = 2
            with self.assertRaises(RunMigrationError) as raised:
                migrate_run_document(run)
            self.assertEqual(raised.exception.code, "ambiguous_run_admission_metadata")
