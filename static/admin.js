...
function wireAdmin() {
  ...
  $('buildOutputPath')?.addEventListener('input',event=>{window.recipeBuildOutput.path=event.target.value;if(event.target.value.trim())scheduleRecipeAutosave();});
  $('btnAddBuildOutputPath')?.addEventListener('click',()=>addBuildOutputPath());
  $('buildOutputPathList')?.addEventListener('input',event=>{const field=event.target.closest('input[data-output-path-index]');if(!field)return;window.recipeBuildOutput.paths[Number(field.dataset.outputPathIndex)]=field.value;if(field.value.trim())scheduleRecipeAutosave();});
  ...
  $('installMappingList')?.addEventListener('input',event=>{const source=event.target.closest('[data-install-mapping-source]');const destination=event.target.closest('[data-install-mapping-destination]');const field=source||destination;if(!field)return;const index=Number(source?.dataset.installMappingSource ?? destination.dataset.installMappingDestination);const mapping=window.recipeInstallMappings[index];mapping[source?'source':'destination']=field.value;delete mapping.legacy;if(mapping.source.trim()&&mapping.destination.trim())scheduleRecipeAutosave();});
