...
| Runtime dependencies | Readiness, detected/manual/bundled/unresolved counts and effective Depends after Build. | ELF toggle, manual Depends and override reasons, exact SONAME/probe/resolver detail. #30 is scoped to copied amd64 Release-asset payloads; mapping-only installs supply no candidate files to the current analyzer. Test cannot claim final ELF Depends until staging/Build has produced them. |
...
| Runtime/persistent directory with owner/group/mode | Backend supports package-specific `/etc`, `/var/lib`, `/var/log` paths through `install.directories` | UX can expose those supported paths and validate ownership/order through Test. Other paths need a separate mechanism. |
...
Current directory creation is declarative for the paths accepted by `install.directories`, but not automatic inference. That field cannot replace a Zoraxy `/opt` directory workaround or create Pocket-ID `/opt/pocket-id/data`; those paths require a reviewed existing mechanism or a separate backend extension. Pocket-ID’s generated secret is a genuine gap. Existing `build.ensure_directories` solves a different post-build output case and must not be presented as runtime provisioning. Raw maintainer scripts remain supported for exceptional lifecycle logic.
