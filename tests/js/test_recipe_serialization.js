...
const roundTrip = {
  name: 'typed-demo',
  active: false,
  automation: {enabled:true, policy:'build_validate'},
  runtime_apt_repositories: [{id:'vendor', uri:'https://apt.example.invalid/runtime', suite:'stable', components:['main'], signing_key:{armored:'PUBLIC KEY'}}],
...
assert.equal(collected.schema_version, 5);
assert.equal(collected.active, false);
assert.deepEqual(JSON.parse(JSON.stringify(collected.automation)), roundTrip.automation);
assert.equal(nodes.recipeAutomationEnabled.checked, true);
assert.equal(nodes.recipeAutomationPolicy.value, 'build_validate');
assert.deepEqual(JSON.parse(JSON.stringify(collected.runtime_apt_repositories)), roundTrip.runtime_apt_repositories);
