...
**Zoraxy:** GitHub Release raw binary → exact asset → one executable mapped to `/usr/local/bin/zoraxy` → service command and WorkingDirectory `/opt/zoraxy`. The current validator restricts `install.directories` to package-specific `/etc`, `/var/lib`, and `/var/log` paths, so it cannot declare `/opt/zoraxy`. `service.working_directory` alone does not create a directory. A source-backed mapping/payload, reviewed lifecycle hook, or separately approved backend extension must provide that path; the UI must not silently promise it.

**Pocket-ID:** prebuilt executable plus service account. Existing `install.account` supports account provisioning; `install.directories` supports package-specific persistent paths under `/etc`, `/var/lib`, or `/var/log`; `install.config_files` with `create_if_missing` supports source-backed config files. It does not declaratively create `/opt/pocket-id/data`. `service.environment_files` references a file but does not generate it. A generated secret/value and write-once environment-file model requires a bounded backend contract with ownership, mode, persistence, upgrades, purge, validation and preview rules. Do not present it as shipped. The operator supplies app-specific values such as URL and policy flags. Maintainer scripts remain an advanced escape hatch until such a contract is approved.

**#30 runtime libraries:** the Recipe can enable amd64 Release-asset ELF detection and holds manual Depends/overrides; the Build’s staging result supplies detected, manual, bundled, overridden, unresolved and effective Depends. The analyzer inspects copied automatic payload files, not `install.config_files`-only mappings. Present a compact “Runtime libraries: ready / review needed” summary only when this path is applicable, a reviewed effective Depends list, and separate detected/manual/bundled/override rows. Zero requirements should read “No external runtime libraries detected” only after an actual scan. Put SONAME, `readelf`, `dpkg-shlibdeps`, resolver profile and raw override JSON in Advanced/diagnostics. Unresolved requirements block the happy path and explain the safe next action; do not promise automatic resolution beyond the actual backend.
