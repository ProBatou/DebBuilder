...
class StaticUiTests(unittest.TestCase):
    ...
    def test_logs_and_recipe_have_structured_insight_surfaces(self):
        ...
        insight = self.read("static/js/build_insight.js")
        css = self.styles()
        for marker in ('id="executionDiagnostic"', 'id="recipePreflight"', 'id="recipePreflightContent"'):
            self.assertIn(marker, html)
        for marker in ("function renderExecutionDiagnostic", "function renderPreflightReport", "What to do next", "Warnings & blockers", "preflight-archive-payload-facts", "insight-section--service"):
            self.assertIn(marker, insight)
        self.assertIn(".execution-diagnostic", css)
        self.assertIn(".preflight-report", css)
    ...
    def test_upstream_archive_ui_uses_conditional_source_controls(self):
        ...
        app = self.read("static/app.js")
        serialization = self.read("static/recipe_serialization.js")
        tree = self.read("static/js/recipe/archive_tree.js")
        packages = self.read("static/js/pages/packages.js")
        logs = self.read("static/js/pages/logs.js")
        ...
        self.assertIn('id="recipeArchivePayloadField"', html)
        self.assertIn('value="paths"', html)
        self.assertIn('value="entire_archive"', html)
        self.assertIn('archive-payload-modes-title', html)
        self.assertIn('/js/recipe/archive_tree.js', html)
        ...
        self.assertIn("closest?.('[data-archive-action]')", app)
        self.assertIn("aria-expanded", app)
        self.assertIn("Include recursively", app)
        self.assertIn("Exclude recursively", app)
        self.assertIn("archive-summary-card--selected", app)
        self.assertIn("archive-tree-state--excluded", app)
        self.assertNotIn("Included by entire archive", tree)
        self.assertNotIn("Inside included directory", tree)
        self.assertNotIn("files.slice(0, 80)", app)
        ...
        self.assertIn("!state.stale && inspectedSource.payload_kind === 'raw_file'", app)
        self.assertIn("Raw file · ${archiveCount('file', source.file_count || 1)}", app)
        self.assertIn("Stale inspection · ${archiveCount('file', state.inventory.file_count)}", app)
        self.assertIn("row.payload_kind === 'raw_file' ? 'raw file'", app)
        self.assertIn("source.payload_kind === 'raw_file'", packages)
        self.assertIn("Raw file · ${count}", packages)
        self.assertIn("['Source SHA-256', source.sha256]", packages)
        self.assertIn("source.payload_kind === 'raw_file'", logs)
        self.assertIn("['Source payload', sourcePayload]", logs)
        self.assertIn("['Source SHA-256', sourceAsset.sha256", logs)
