...
Cross-object actions pass both the destination and the fixture object's stable identity: Package name, Recipe ID or Run ID. An explicit Run target takes precedence over scenario defaults and previous selection. The Package detail offers View Run and Review Recipe only when its fixture has the corresponding ID. Mobile Package Back sits outside the detail card and restores focus to the selected list row.
...
The log Options panel stays open for controls inside it, closes on an outside pointer action, and closes on Escape while returning focus to its trigger.
...
Top-level navigation still focuses the new page title for announcement. The title outline follows `:focus-visible`: keyboard activation shows it, while pointer activation does not.
