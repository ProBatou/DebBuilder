...
const stale = ArchiveTree.createState({mode:'paths', include:['server.js', 'foo/a.txt'], exclude:[]});
ArchiveTree.setInventory(stale, inventory);
ArchiveTree.markStale(stale);
const before = JSON.stringify(stale.payload);
assert.equal(ArchiveTree.includePath(stale, 'tests/test.js'), false);
assert.equal(ArchiveTree.excludePath(stale, 'tests/'), false);
assert.equal(JSON.stringify(stale.payload), before);
assert.ok(ArchiveTree.selectionSummary(stale).explicitFiles > 0);
ArchiveTree.setInventory(stale, inventory);
assert.equal(ArchiveTree.includePath(stale, 'tests/test.js'), true);
