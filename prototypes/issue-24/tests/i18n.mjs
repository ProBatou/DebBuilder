...
const allow=new Set(['D','DebBuilder','stable · amd64','Luminous · amd64','Luminous · main · amd64','Luminous','v1.0.0','stable · main · amd64','stable','main','amd64','amd64 · all','repository.gpg · InRelease','InRelease','v3.1.4 · zoraxy_linux_amd64','libc6, libssl3','ca-certificates','libc6, libssl3, ca-certificates','debbuilder · 1.0.0']);
const components=['App.svelte',...(await readdir(path.join(root,'pages'))).filter(x=>x.endsWith('.svelte')).map(x=>'pages/'+x),...(await readdir(path.join(root,'lib'))).filter(x=>x.endsWith('.svelte')).map(x=>'lib/'+x)];
