...
function wireAdmin() {
  ...
  $('installMappingList')?.addEventListener('input',event=>{const source=event.target.closest('[data-install-mapping-source]');const destination=event.target.closest('[data-install-mapping-destination]');const policy=event.target.closest('[data-install-mapping-policy]');const field=source||destination||policy;if(!field)return;const index=Number(source?.dataset.installMappingSource ?? destination?.dataset.installMappingDestination ?? policy.dataset.installMappingPolicy);const mapping=window.recipeInstallMappings[index];mapping[source?'source':destination?'destination':'policy']=field.value;if(mapping.source.trim()&&mapping.destination.trim())scheduleRecipeAutosave();});
